...
const cycleRules: CycleRule[] = (cyclesJson as CycleRule[])
  .slice()
  .sort((a, b) => a.cycle - b.cycle);

const seRules: SeEffectRule[] = (techsSeEffectJson as SeEffectRule[])
  .slice()
  .sort((a, b) => a.se - b.se);

const techs: TechData[] = (techsJson as TechData[])
  .slice()
  .sort((a, b) => a.id - b.id);
...
export function calculateZatBoostPerTech(
  cycles: number,
  mode: ZatMode,
): number {
...
export function calculateNextThreeTechCosts(
  id: number,
  currentLevel: number,
): TechCostEntry[] {
  ...
  for (let level = 1; level <= targetLevel; level += 1) {
    while (
      curveIndex < sortedCurve.length &&
      sortedCurve[curveIndex].level === level
    ) {
...
export function calculateTechBoost(
  zatBoost: number,
  seEffect: number,
  id: number,
  mode: ZatMode,
...
export function calculateTotalPremiumMultiplier(
  purchases: PremiumInput,
): PremiumSummary {
  let additive = 0;
...
export function calculateNextZatCost(
  junoAmount: LargeNumber | string | number,
): NextZatCostResult {
