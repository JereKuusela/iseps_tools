...
export class LargeNumber {
  ...
  static from(input: string | number | LargeNumber): LargeNumber {
    if (input instanceof LargeNumber) return new LargeNumber(input.mantissa, input.exponent)
  ...
  multiply(otherInput: string | number | LargeNumber): LargeNumber {
    ...
    return new LargeNumber(this.mantissa * other.mantissa, this.exponent + other.exponent)
  }
