...
describe("calculateNextThreeTechCosts", () => {
  it("returns next three levels and increasing costs", () => {
    ...
    const first =
      Number(result[0].cost.mantissa) * 10 ** result[0].cost.exponent;
    const second =
      Number(result[1].cost.mantissa) * 10 ** result[1].cost.exponent;
    const third =
      Number(result[2].cost.mantissa) * 10 ** result[2].cost.exponent;
