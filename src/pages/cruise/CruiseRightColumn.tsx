          ...
          <div class="overflow-hidden rounded-xl border border-ink/10 bg-white dark:border-white/15 dark:bg-[#22344d]">
            <div class="space-y-2 border-b border-ink/10 px-2.5 py-2 dark:border-white/10">
              <p class="text-xs font-semibold text-ink/80 dark:text-white/80">
                Your build is {optimalityPercentLabel()}
              </p>
                    ...
                    >
                      ...
                      <td class="px-2 py-1">
                        <input
                          ...
                          class="w-14 rounded border border-ink/20 bg-white px-1.5 py-0.5 text-right text-xs font-medium text-ink outline-none ring-brand/40 transition focus:ring disabled:cursor-not-allowed disabled:bg-ink/5 dark:border-white/15 dark:bg-[#1a2638] dark:text-white"
                        />
                      </td>
                      <Show when={showOptimalColumn()}>
                        <td class="px-2 py-1">
                      ...
                      <td class="px-2 py-1 text-ink/80 dark:text-white/80">{row.nextCost ?? "-"}</td>
                      <td class="px-2 py-1 text-ink/80 dark:text-white/80">
                        {row.nextCost ? formatCompactMultiplier(row.nextBonusMultiplier) : "-"}
                      </td>
                      <td class="px-2 py-1 text-ink/80 dark:text-white/80">
                        {row.nextCost ? formatPercentFromRatio(row.relativeBonusPerPoint, 0) : "-"}
                      </td>
                    </tr>
                  )}
                </For>
