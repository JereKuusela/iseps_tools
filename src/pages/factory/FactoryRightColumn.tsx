...
export const FactoryRightColumn = () => {
  const factory = useFactoryContext()

  const bestRow = createMemo(() => {
    const winner = factory.bestNodeId()
    if (!winner) return null
    return factory.evaluationRows().find((row) => row.id === winner) ?? null
  })

  const optimalLevels = createMemo(() => factory.optimalLevels())
  const showOptimalColumn = () => !factory.isOptimal()
  const optimalityPercentLabel = createMemo(() => `${factory.optimality()} optimal.`)
    ...
    <div class="grid gap-3">
      <div class="overflow-hidden rounded-xl border border-ink/10 bg-white dark:border-white/15 dark:bg-[#22344d]">
        ...
        <table class="w-full table-fixed border-collapse text-left text-xs sm:text-sm">
          ...
          <tbody>
            <Index each={factory.evaluationRows()}>
              {(row) => (
                <tr
                  class="border-t border-ink/10 dark:border-white/10"
                  classList={{
                    "bg-[#12a89d]/10 dark:bg-[#8ce3ff]/10": factory.bestNodeId() === row().id && row().affordable,
                ...
                >
                  ...
                  <td class="px-2 py-1">
                    <input
                      ...
                      onInput={(event) =>
                        factory.setNodeLevel(row().id, Math.floor(parseNumberish(event.currentTarget.value)))
                      }
