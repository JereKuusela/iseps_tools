...
export const TokensLevelInputs = () => {
  ...
  const renderCell = (group: "output" | "supplies" | "bbbot", resource: string) => {
    if (group === "output") {
      ...
      const hasOutput = tokens.upgrades().some((upgrade) => upgrade.id === `output.${resource}`)
      if (!hasOutput) return <span class="text-ink/40 dark:text-white/40">-</span>
      ...
      return (
        <input
          ...
          step={1}
          disabled={!unlocked}
          onInput={(event) => tokens.setOutputLevelByResource(resource, event.currentTarget.value)}
    ...
    return (
      <input
        ...
        step={1}
        disabled={!unlocked}
        onInput={(event) => tokens.setUpgradeLevel(id, event.currentTarget.value)}
        class="w-[72px] rounded border border-ink/20 bg-white px-1.5 py-0.5 text-right text-xs font-medium text-ink outline-none ring-brand/40 transition focus:ring disabled:cursor-not-allowed disabled:opacity-40 dark:border-white/15 dark:bg-[#1a2638] dark:text-white"
      />
    )
  }
