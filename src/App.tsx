...
function TopNav() {
  ...
  return (
    <header class="relative z-10 px-4 pt-5 sm:px-8 sm:pt-8 lg:px-10">
      <div class="mx-auto grid w-full max-w-6xl gap-2.5 rounded-2xl border border-white/60 bg-white/70 p-2.5 shadow-glow backdrop-blur sm:grid-cols-3 sm:gap-3 sm:p-3">
        <For each={tabs}>
          {(tab) => (
            <A
              href={tab.href}
  ...
  return (
    <div class="relative min-h-screen overflow-hidden bg-mist text-ink">
      ...
      <main class="mx-auto w-full max-w-6xl px-4 pb-10 pt-4 sm:px-8 sm:pb-12 sm:pt-6 lg:px-10 lg:pb-14 lg:pt-8">
        <Switch
          fallback={<OgTechPage cycles={cycles()} setCycles={setCycles} />}
        >
