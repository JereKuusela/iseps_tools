  ...
  return (
    <label class="flex cursor-pointer items-start justify-between gap-3 rounded-xl border border-ink/15 bg-white/70 px-3 py-2.5 transition hover:border-ink/30">
      <div>
        ...
        {props.hint ? <p class="mt-0.5 text-xs text-ink/60">{props.hint}</p> : null}
      </div>
      <input
        type="checkbox"
...
export function SelectField(props: SelectFieldProps) {
  return (
    <div class="grid gap-1.5">
      <div class="flex items-center gap-2">
        <label class="text-xs font-semibold uppercase tracking-[0.12em] text-ink/75">{props.label}</label>
        {props.tooltip ? <Tooltip content={props.tooltip} /> : null}
      </div>
      <select
        value={props.value}
